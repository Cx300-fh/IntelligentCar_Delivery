...
uint8_t lq_i2c_lsm6dsr_init(struct ls_i2c_dev *dev)
{
    ...
    delay_ms(50);
    i2c_write_reg(dev, LSM6DSR_CTRL3_C , 0x80);         // 关闭LSM6DSR控制寄存器组
    delay_ms(50); 
    i2c_write_reg(dev, LSM6DSR_CTRL3_C , 0x44);         // 关闭LSM6DSR控制寄存器组
    delay_ms(50);   // 延时等待传感器启动完成
   ...
   // lsm6dsr_setacc_fullscale(dev, ACC_FS_XL_2G);         // acc,2g量程，分辨率高
    i2c_write_reg(dev, LSM6DSR_CTRL9_XL , 0x02);         // 使能加速度计x, y, z轴
    i2c_write_reg(dev, LSM6DSR_CTRL1_XL , 0x37); 
    i2c_write_reg(dev, LSM6DSR_CTRL2_G , 0x37);
    i2c_write_reg(dev, LSM6DSR_CTRL6_C , 0x05);
    i2c_write_reg(dev, LSM6DSR_CTRL8_XL , 0x00);
