...
int main()
{
    ...
    delivery.Init();
    delivery.Start();
    // 配送模式开机不再强制切DConfirm页：保留原有页面交互
    // （主页START进配送页/退出按钮回主页，页面切换由HMI按钮事件完成，见screen.cpp）
    ...
    while (ls_system_running.load())
    {
        ...
        // 屏幕数据：
        // - 本地模式：每10圈发旧调试页数据（原行为不变）
        // - 配送模式：DConfirm页由Screen_Render_Delivery负责（内部页面门控+节流）；
        //   其余页面（主页/地图/WiFi页）仍要刷新基础数据——WiFi状态/IP等只在Screen_Send_All里发
        static int screen_send_cnt = 0;
        bool send_all_now = false;
        if (delivery.Enabled()) {
            Screen_Render_Delivery();
            static uint32_t all_data_ms = 0;
            if (!screen_delivery_page_active && lq_get_tick_ms() - all_data_ms >= 1000) {
                all_data_ms = lq_get_tick_ms();
                send_all_now = true;
            }
