...
struct NavStatus {
    ...
    // 路线偏离标志
    // 0=未知/默认, 1=正常路线(OK), 2=偏离路线(ERROR)
    uint8_t route_flag;

    // 语音播报标志
    bool arrived_announced;  // 是否已播报到达终点
    bool voice_announced;    // 是否已播报当前位置
    int last_announced_id;   // 上一次播报的节点ID
...
public:
    ...
    // ==================== 避障绕行（阻塞边重规划） ====================
    // 检测到障碍物挡住 from-to 这条道路时调用：标记该边阻塞并重新规划，
    // 不取消当前任务（不调用cancel_task）。任务未激活时只记录阻塞，供之后规划参考。
    // 无法绕行（无其他路径）时返回false并停车，任务保留，等待道路解除阻塞后重试。
    bool reroute_blocked_edge(int from, int to);
...
private:
    ...
    void calculate_path_distances(int current_id, int target_id, int* dist);
    // 状态处理函数
    ...
    // 重新规划路径，并更新路径索引和下一个目标ID
    bool replan_path(void);
