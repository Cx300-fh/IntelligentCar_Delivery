 ...
 */
Dijkstra::Dijkstra(int map_id) : weight_provider(nullptr) {
    clear_weight_tables();
    clear_all();
    init_map(map_id);
}

void Dijkstra::clear_weight_tables(void) {
    for (int m = 0; m < 3; m++) {
        for (int i = 0; i < MAX_NODE_COUNT; i++) {
            for (int j = 0; j < MAX_NODE_COUNT; j++) {
                edge_manual_penalty[m][i][j] = 0;
                edge_dynamic_penalty[m][i][j] = 0;
                edge_blocked[m][i][j] = false;
            }
        }
    }
}
...
void Dijkstra::init_map(int map_id) {
    clear_all();
    if (map_id == MAP_THU) init_thu();
    else if (map_id == MAP_SUTD) init_sutd();
    else current_map = MAP_NONE;
}
...
void Dijkstra::init_thu(void) {
    ...
    add_undirected_edge(1, 3, 65);
    add_undirected_edge(1, 5, 65);

    // 理科楼(2)的邻近点：图书馆(3)、校医院(6)
    add_undirected_edge(2, 3, 75);
    add_undirected_edge(2, 6, 92);
    ...
    // (1-5, 4-5已在上面添加)
    add_undirected_edge(5, 8, 96);

    // 校医院(6)的邻近点：理科楼(2)、大礼堂(9)
    // (2-6已在上面添加)
    add_undirected_edge(6, 9, 162);
...
static const int thu_coords[MAX_NODE_COUNT][2] = {
    {0,0}, {2250,1300}, {650,650}, {1400,650}, {2250,650}, {3100,650}, {0,0},
    {2250,0}, {3850,0}, {1400,-650}, {2250,-650}, {3100,-650}, {3850,-650},
    {2250,-1300}, {3100,-1300}, {0,0}
};
// SUTD 节点坐标
static const int sutd_coords[MAX_NODE_COUNT][2] = {
    {0,0}, {0,-1600}, {1350,0}, {-1250,-800}, {-1250,0}, {0,0}, {0,1450},
    {-625,-800}, {-625,0}, {0,750}, {0,-800}, {650,1050}, {650,0}, {0,0}, {0,0}, {0,0}
};
...
void Dijkstra::add_undirected_edge(int from, int to, int weight) {
    ...
    if (current_map >= MAP_NONE && current_map <= MAP_SUTD) {
        graph[from].back().manual_penalty = edge_manual_penalty[current_map][from][to];
        graph[from].back().dynamic_penalty = edge_dynamic_penalty[current_map][from][to];
        graph[from].back().blocked = edge_blocked[current_map][from][to];
        graph[to].back().manual_penalty = edge_manual_penalty[current_map][to][from];
        graph[to].back().dynamic_penalty = edge_dynamic_penalty[current_map][to][from];
        graph[to].back().blocked = edge_blocked[current_map][to][from];
    }
}

void Dijkstra::set_dynamic_weight_provider(DynamicWeightProvider* provider) {
    weight_provider = provider;
}

bool Dijkstra::set_edge_penalty(int from, int to, int manual_penalty, int dynamic_penalty, bool blocked) {
    if (from <= 0 || from >= active_node_count || to <= 0 || to >= active_node_count) return false;
    if (current_map < MAP_NONE || current_map > MAP_SUTD) return false;

    int safe_manual = manual_penalty < 0 ? 0 : manual_penalty;
    int safe_dynamic = dynamic_penalty < 0 ? 0 : dynamic_penalty;
    edge_manual_penalty[current_map][from][to] = safe_manual;
    edge_manual_penalty[current_map][to][from] = safe_manual;
    edge_dynamic_penalty[current_map][from][to] = safe_dynamic;
    edge_dynamic_penalty[current_map][to][from] = safe_dynamic;
    edge_blocked[current_map][from][to] = blocked;
    edge_blocked[current_map][to][from] = blocked;

    bool updated = false;
    for (Edge& e : graph[from]) {
        if (e.to == to) {
            e.manual_penalty = safe_manual;
            e.dynamic_penalty = safe_dynamic;
            e.blocked = blocked;
            updated = true;
            break;
        }
...
int Dijkstra::get_edge_cost(int from, int to, WeightMode mode) {
    if (from <= 0 || from >= active_node_count || to <= 0 || to >= active_node_count) return INF;

    for (const Edge& e : graph[from]) {
        if (e.to != to) continue;
        if (mode == WEIGHT_BASE) return e.base_weight;

        bool blocked = e.blocked;
        int dynamic_penalty = e.dynamic_penalty;
        if (weight_provider) {
            blocked = blocked || weight_provider->is_edge_blocked(current_map, from, to);
            int provider_penalty = weight_provider->get_dynamic_penalty(current_map, from, to);
            if (provider_penalty > dynamic_penalty) dynamic_penalty = provider_penalty;
        }
        if (blocked) return INF;

        int manual_penalty = e.manual_penalty < 0 ? 0 : e.manual_penalty;
        if (dynamic_penalty < 0) dynamic_penalty = 0;
        return e.base_weight + manual_penalty + dynamic_penalty;
    }
 ...
 */
int Dijkstra::find_shortest_path(int start, int end, WeightMode mode) {
    ...
    for (int i=0;i<active_node_count;i++){
        ...
        for(const Edge& e:graph[u]) {
            int edge_cost = get_edge_cost(u, e.to, mode);
            if(edge_cost >= INF) continue;
            if(!visited[e.to]&&dist[u]+edge_cost<dist[e.to]){dist[e.to]=dist[u]+edge_cost;prev[e.to]=u;}
        }
    }
 ...
 */
int Dijkstra::get_path(int start, int end, int* path, WeightMode mode) {
    if(!path) return 0;
    if(find_shortest_path(start,end,mode)==-1) return 0;
...
int Dijkstra::get_path_cost(const int* path, int path_len, WeightMode mode) {
    if (!path || path_len <= 0) return -1;
    if (path_len == 1) return 0;

    int total = 0;
    for (int i = 0; i < path_len - 1; i++) {
        int cost = get_edge_cost(path[i], path[i + 1], mode);
