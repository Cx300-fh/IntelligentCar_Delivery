...
void Screen_Send_All(void)
{
    ...
    // 目标速度
    if (run==1) {
        Screen_Send_Var("n_speed", (int)target_speed);
    } else {
    ...
    if (nav_task.active) {
        ...
        // 当前动作 -> t_action
        if (order_scheduler.has_pending_station_action()) {
            Screen_Send_Text("t_state", order_scheduler.get_pending_action_text_en());
            Screen_Send_Text("t_action", order_scheduler.get_pending_action_text_en());
        } else {
        ...
        bool off_route = false;  // 是否偏离路线标志
        // 是否偏离路线 -> t_route
        // 判断逻辑：当前节点是否等于预期的节点
        // expected_next_id 表示"当前应该站在哪个节点"
        // - 到达节点后、离开前：expected_next_id = current_id（显示OK）
        // - 离开节点后：expected_next_id = next_id（等待到达下一个节点）
        if (nav_status.current_id > 0 && nav_status.expected_next_id > 0) {
            if (nav_status.current_id == nav_status.expected_next_id) {
                // 当前节点是检测前预期的节点，正常
        ...
        // 规划距离显示 -> n_cur_d(数值控件), n_back_d(数值控件)
        // n_cur_d: 当前位置到目标的直行距离(cm)，用于显示剩余距离
        // n_back_d: 返航距离(cm)，偏离路线时计算回到正轨需要的距离，未偏离时显示9999表示无需返航
        if (nav_status.current_id > 0 && nav_task.target_id > 0) {
            // 计算当前位置到目标的直行距离（Dijkstra最短路径）
            int direct_dist = nav_fsm.get_dijkstra()->find_shortest_path(
                nav_status.current_id, nav_task.target_id);
            Screen_Send_Var("n_cur_d", direct_dist > 0 ? direct_dist : 0);

            // 返航距离计算：只有在偏离路线时才需要计算返回上一节点的距离
            // 返航距离 = 当前位置到上一节点的距离 + 上一节点到目标的距离
            // 未偏离时显示9999表示在正常路线上，无需返航
            if (off_route && nav_status.prev_id > 0) {
                // 计算两段距离：当前位置->上一节点，上一节点->目标
                int back_dist = nav_fsm.get_dijkstra()->find_shortest_path(
                    nav_status.current_id, nav_status.prev_id);
                int prev_to_target = nav_fsm.get_dijkstra()->find_shortest_path(
                    nav_status.prev_id, nav_task.target_id);
                if (back_dist > 0 && prev_to_target > 0) {
                    // 路径有效，计算总返航距离
                    int total_back = back_dist + prev_to_target;
                    Screen_Send_Var("n_back_d", total_back);
                } else {
                    // 路径计算失败，显示9999表示无法计算返航距离
...
static bool parse_four_int_command(const char* cmd, const char* prefix, int* a, int* b, int* c, int* d)
{
    int prefix_len = strlen(prefix);
    if (strncmp(cmd, prefix, prefix_len) != 0) return false;
    if (cmd[prefix_len] != ',') return false;
    return sscanf(cmd + prefix_len + 1, "%d,%d,%d,%d", a, b, c, d) == 4;
}
...
void Screen_Rx_Process(const uint8_t* data, ssize_t len)
{
    ...
    // 4. 订单调度测试指令
    // 格式:
    //   ORDER,order_id,map_id,pickup_node,dropoff_node
    //   CONFIRM
    //   EDGE,from,to,manual_penalty,dynamic_penalty,blocked
    int order_id = 0;
    int pickup_node = 0;
    int dropoff_node = 0;
    if (parse_four_int_command(cmd, "ORDER", &order_id, &map_id, &pickup_node, &dropoff_node))
    {
        printf("[Screen] RX: %s\n", cmd);
        if (order_scheduler.submit_order(order_id, map_id, pickup_node, dropoff_node)) {
            Screen_Send_Text("t_route", get_scheduler_event_name(order_scheduler.get_last_event().type));
        } else {
            Screen_Send_Text("t_route", "ORDER ERROR");
        }
        return;
    }

    if (strcmp(cmd, "CONFIRM") == 0 || strcmp(cmd, "confirm") == 0)
    {
        printf("[Screen] RX: %s\n", cmd);
        if (order_scheduler.confirm_station_action()) {
            Screen_Send_Text("t_route", "CONFIRMED");
        } else {
            Screen_Send_Text("t_route", "NO ACTION");
        }
        return;
    }

    if (strncmp(cmd, "EDGE,", 5) == 0)
    {
        int from = 0, to = 0, manual_penalty = 0, dynamic_penalty = 0, blocked = 0;
        printf("[Screen] RX: %s\n", cmd);
        if (sscanf(cmd + 5, "%d,%d,%d,%d,%d", &from, &to, &manual_penalty, &dynamic_penalty, &blocked) == 5 &&
            order_scheduler.update_edge_condition(from, to, manual_penalty, dynamic_penalty, blocked != 0)) {
            Screen_Send_Text("t_route", "EDGE UPDATED");
        } else {
            Screen_Send_Text("t_route", "EDGE ERROR");
        }
        return;
    }
